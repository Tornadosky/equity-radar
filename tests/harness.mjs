...
export function loadRadar(overrides={}, {boot=false, source}={}) {
  const nodes = new Map();
  ...
  const sourceText=fs.readFileSync(source || new URL('../public/assets/radar.js',import.meta.url),'utf8');
  const names=['render','openDetails','document','verifyMarketPnls','refreshSettlementData','feeConfigForMarket','traderDiagnostics','state','els','feeForTrade','calculateRoundSettlement','marketWinner','inferWinnerFromPositions','enrichGroups','buildGroups','prepareTradeBatch','mergeTakerTrades','classifyTakerTrades','buildCumulativeSeries','calculateMaxDrawdownFromPoints','getVisibleRows','getDailyChartRows','buildCsv','fetchActivityHistory','fetchRecentActivity','fetchTakerTradeHistory','tradeFingerprint','normalizeTradeRecord','runCalculationSelfTest','demoData','isBtc5m','notionalEvidence','tradeNotional','normalizeTimeframe','getTimeframe','isSupportedMarket','isSelectedMarket','marketDescriptor','getStartSec','formatRange','renderCurrentRound','setMarketSelection','syncMarketLabels','restoreMarketSelection','selectionLabel','pollLiveTrades','showDemo','loadReport','initializeTradeState','mergeLiveTrades','exportCsv','loadDailyRebates','invalidateRebates','resolveProfile'];
  const end=boot?sourceText.lastIndexOf("})();"):sourceText.indexOf("  els.load.addEventListener");
  const api=vm.runInContext(sourceText.slice(0,end)+`return {${names.join(',')}};\n})();`,context);
  return {...api, document};
}
