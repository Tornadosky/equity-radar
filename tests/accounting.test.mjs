...
test('both-side filter applies to chart as well as table',()=>{const a=loadRadar();a.state.rows=[{resolved:true,pnl:2,bothSides:true,endSec:1},{resolved:true,pnl:3,bothSides:false,endSec:2}];a.els.bothOnly.checked=true;assert.equal(a.getDailyChartRows().length,1);});
